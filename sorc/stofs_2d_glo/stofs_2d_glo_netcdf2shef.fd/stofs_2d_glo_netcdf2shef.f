!--------------------------------------------------------------------------
! stofs_2d_glo_netcdf2shef.f is to compute shef output file in feet from
...
!------    Read ESTOFS MSL-MLLW datum difference  -------
   ...
   13 format(A18,X,I5)
      allocate(xy3(np))
      do i = 1, np
         read(5,'(A7,x,F5.3)') msl2mllw, xy3(i)
      end do
...
C******* Convert meter to feet *********
      ...
      ncsth = 60 ! 60 time step nowcast output
      ihead = 4 ! number of output at header
      irow = 12 ! number of rows
      iout = 5 ! 1: every 6 minutes, 5: every 30 minutes
      lmn=5000
      do i = 1, station
   ...
   16    format(".E",X,A5,X,I4,2I2.2,X,"Z",X,"DH",2I2.2,"/DC",4I2.2,
     &          "/HMIFC","/DIN",I2.2,"/",X,4(F8.2,"/",X),F8.2)
