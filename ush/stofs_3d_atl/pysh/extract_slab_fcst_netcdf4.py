...
if __name__ == '__main__':
    ...
    '''
    Example Usage: python extract_slab_fcst_netcdf4.py --date 2000-01-01-12 --stack 1
    (extract from the 1st stack, *_1.nc)
    Run this script in the schism run directory, whichi should include:

    Input:
        1. Assign work directory path (default: fpath='.')
    ...
    argparser.add_argument('--stack', type=int, required=True)
    args = argparser.parse_args()

    # -------------------------- input paramters  ----------------------------------
    # Input (1), command line input: date (e.g., 2000-01-01, yyyy-mm-dd-HH)
    startdate=args.date

    # Input (3), command line input: stack number
    sid = args.stack

    # Input (3), work directory, which should be a schism run dir
    fpath = "."

    # Input (4), output directory
    # results= './results/'
    results= './results'
    # ----------------------- end input parameters------------------------------

    print(f'seconds since {startdate.year}-{startdate.month:02d}-{startdate.day:02d} {startdate.hour:02d}:00:00 UTC')
    print(f'{startdate.year}-{startdate.month:02d}-{startdate.day:02d} {startdate.hour:02d}:00:00 UTC')
    ...
    with Dataset(f"{results}/schout_2d_{sid}.nc", "w", format="NETCDF4") as fout:
        #dimensions
