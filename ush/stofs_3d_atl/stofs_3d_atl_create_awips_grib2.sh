        ...
        if [ $err -eq 0 ]; then
           ...
           if [ $SENDDBN = YES ]; then
             ...
             for fn_grb in ${list_conus}; do            
                $DBNROOT/bin/dbn_alert MODEL STOFS_NETCDF $job ${COMOUT}/${fn_grb}
                 export err=$?; err_chk
             done
             ...
             $DBNROOT/bin/dbn_alert MODEL STOFS_NETCDF $job ${COMOUT}/${RUN}.${cycle}.conus.east.cwl.grib2
             export err=$?; err_chk

             $DBNROOT/bin/dbn_alert MODEL STOFS_NETCDF $job ${COMOUT}/${RUN}.${cycle}.puertori.cwl.grib2
             export err=$?; err_chk
