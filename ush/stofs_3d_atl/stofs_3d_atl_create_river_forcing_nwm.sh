...
# cp files to work dir
  ...
  cpreq -f ${fn_source_scale}           ${dir_wk}/source_scale.txt
  cpreq -f ${fn_pump_sinks}           ${dir_wk}/pump_sinks.txt 
  cpreq -f $fn_featureID_source_idx  ${dir_wk}/featureID_source.idx
 ...
 if [[ ${N_rows_riv_th_ori} -ge ${N_list_min} ]]; then
   ...
   if [[ ${N_rows_riv_th_ori} -lt ${N_list_target} ]]; then
      ...
      do
        ...
        cpreq -pf ${fn_river_th} ${COMOUTrerun}/${fn_river_th_std}
