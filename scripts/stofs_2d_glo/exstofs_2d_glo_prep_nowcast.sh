...
# 2.  Get hotstart time and file 
  ...
  $USHstofs/${RUN}_multistart.sh "hotstart" >> $pgmout 2>errfile
  export err=$?; err_chk
  ...
  hcyc=`echo $ymdh | cut -c9-10`
  hdir=$COM/${RUN}.${hdate}
  hfile=${RUN}.t${hcyc}z.hotstart

  if [ -d $hdir ]; then
     if [ -f $hdir/$hfile ]; then
        cpreq $hdir/$hfile ${time_beg}.hotstart
	if [ $cyc = "00" ]; then
           if [ -f $hdir/database.tar.gz ]; then
              cpreq $hdir/database.tar.gz $COMIN/.
	   fi
	fi
     fi
  fi
