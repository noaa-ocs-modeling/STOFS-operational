...
# 2.  Get restart time and file 
  ...
  $USHstofs/stofs_2d_glo_multistart.sh "restart" >> $pgmout 2>errfile
  export err=$?; err_chk
  ...
  rcycle=t`echo $ymdh | cut -c9-10`z
  rdir=${COM}/${RUN}.${rdate}
  rfile=${RUN}.${rcycle}.restart
...
# --------------------------------------------------------------------------- #
# 4.  Create surface forcing for nowcast 

  export pgm="stofs_2d_glo_surface_forching.sh for nowcast"
  . prep_step
  startmsg
  mpiexec -n 1 -ppn 1 $USHstofs/${RUN}_surface_forcing.sh "surface" "$time_beg" "$time_now" >> $pgmout 2>errfile
  export err=$?; err_chk
...
# --------------------------------------------------------------------------- #
# 5.  Set time and file for surface forcing nowcast
# 5.a Set parameters for surface forcing nowcast input file
...
# --------------------------------------------------------------------------- #
# 6.  Execute stofs_2d_glo_adcprep for surface forcing nowcast
...
# --------------------------------------------------------------------------- #
# 7.  Execute stofs_2d_glo_padcirc for surface frocing nowcast
