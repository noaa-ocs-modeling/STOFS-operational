...
# 3.a create awips shef output
  ...
  for type in cwl htp swl; do
      ...
      if [ ${type} = "cwl" ]; then
         $USHstofs/make_ntc_file.pl SXUS02 KWBM ${YMDH} NONE tmp.dat shef_${RUN}.${cycle}.points.${type} >> $pgmout 2>errfile
         export err=$?; err_chk
      elif [ ${type} = "htp" ]; then
         $USHstofs/make_ntc_file.pl SXUS01 KWBM ${YMDH} NONE tmp.dat shef_${RUN}.${cycle}.points.${type} >> $pgmout 2>errfile
         export err=$?; err_chk
      else
         $USHstofs/make_ntc_file.pl SXUS03 KWBM ${YMDH} NONE tmp.dat shef_${RUN}.${cycle}.points.${type} >> $pgmout 2>errfile
         export err=$?; err_chk
      fi
      rm tmp.dat
  done
...
#
  ...
  for type in cwl htp swl; do
      ...
      echo "$EXECstofs/${RUN}_netcdf2grib conus $type $YMDH $FIXstofs/${RUN}_conus.east.mask ${type}.fort.63.nc 3000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib conus $type $YMDH $FIXstofs/${RUN}_conus.west.mask ${type}.fort.63.nc 4000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib puertori $type $YMDH $FIXstofs/${RUN}_puertori.mask ${type}.fort.63.nc 5000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib alaska $type $YMDH $FIXstofs/${RUN}_alaska.mask ${type}.fort.63.nc 6000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib hawaii $type $YMDH $FIXstofs/${RUN}_hawaii.mask ${type}.fort.63.nc 7000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib guam $type $YMDH $FIXstofs/${RUN}_guam.mask ${type}.fort.63.nc 8000 >> $pgmout 2>errfile" >> poescript
      echo "$EXECstofs/${RUN}_netcdf2grib northpacific $type $YMDH $FIXstofs/${RUN}_northpacific.mask ${type}.fort.63.nc 9000 >> $pgmout 2>errfile" >> poescript
