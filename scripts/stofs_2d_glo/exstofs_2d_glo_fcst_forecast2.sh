...
# 4.  Create surface forcing for forecast
  ...
  mpiexec -n 1 -ppn 1 $USHstofs/${RUN}_surface_forcing.sh "surface" "$time_half" "$time_end" >> $pgmout 2>errfile
  export err=$?; err_chk

  if [ -e "$pgmout" ] ; then
     echo "${RUN}_surface_forcing.sh forecast standout log - for "$time_half" "$time_end":"
     cat $pgmout
