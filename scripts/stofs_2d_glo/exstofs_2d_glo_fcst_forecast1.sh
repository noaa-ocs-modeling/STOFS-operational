...
#                                                                             #
###############################################################################
# Start of stofs_2d_glo_fcst_forecast1.sh.ecf script ------------------------ #
# 0.  Preparation
# 0.a Basic modes of operation
  ...
  msg="Starting stofs_2d_glo_fcst_forecast1 scripts"
  echo "$msg"
...
# 4.  Create surface forcing for forecast
  ...
  mpiexec -n 1 -ppn 1 $USHstofs/${RUN}_surface_forcing.sh "surface" "$time_now" "$time_half" >> $pgmout 2>errfile
  export err=$?; err_chk

  if [ -e "$pgmout" ] ; then
     echo "${RUN}_surface_forcing.sh forecast standout log - for "$time_now" "$time_half":"
     cat $pgmout
     echo "${RUN}_surface_forcing.sh forecast errfile log:"
     cat ./errfile
     echo "Done concatenating $pgmout and errfile for ${RUN}_surface_forcing.sh forecast"
  fi
...
# 5.a Set parameters for surface forcing forecast input file
  ...
  rnday=$(echo "scale=5; $fcstd+$lsth/24" | bc)
  rnday_half=$(echo "scale=5; $rnday - $lsth/48" | bc)
  nout=5
...
# 8.  Send files to $COM
  ...
  if [ `expr $(echo "scale=0; $time_fcst/($wndh*3600)" | bc) % 2` = 0 ]; then
  ...
  msg="Completing stofs_2d_glo_fcst_forecast1 script"
  echo "$msg"
  postmsg "$jlogfile" "$msg"

# End of stofs_2d_glo_fcst_forecast1.sh.ecf script ---------------------------- #
