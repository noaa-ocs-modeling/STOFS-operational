...
#                                                                             #
###############################################################################
# Start of stofs_2d_glo_prep_forecast.sh.ecf script ------------------------- #
# 0.  Preparation
# 0.a Basic modes of operation
  ...
  msg="Starting stofs_2d_glo_prep_forecast scripts"
  echo "$msg"
...
# 4.a Set parameters for tide-only forecast input file
  ...
  hotstart=$(grep [0-9]  hottime.out | awk '{print $1}') 
  time_hotstart=$(grep [0-9] hottime.out | awk '{print $2}')
  touts=$(grep [0-9] hottime.out | awk '{print $3}')
  toutf=$(grep [0-9] hottime.out | awk '{print $4}')
  echo $hotstart $time_hotstart $touts $toutf

  while [ $hotstart -le 2 ]; 
  do
     if [ $hotstart -gt 0 ]; then 
        cpreq $COMOUTrerun/${RUN}_hottime.out hottime.out
        hotstart=$(grep [0-9]  hottime.out | awk '{print $1}') 
        time_hotstart=$(grep [0-9] hottime.out | awk '{print $2}')
        touts=$(grep [0-9] hottime.out | awk '{print $3}')
     ...
     echo $hotstart $time_hotstart $touts $toutf > hottime.out
...
# 5.  Execute stofs_2d_glo_adcprep for tide-only forecast
     ...
     export pgm="stofs_2d_glo_adcprep for tide-only forecast${hotstart}"
     . prep_step
     startmsg
...
# 6.  Execute stofs_2d_glo_padcirc for tide-only forecast
     ...
     export pgm="stofs_2d_glo_padcirc for tide-only forecast${hotstart}"
     . prep_step
     startmsg
...
# 7.  Send files to $COM
     ...
     if [ `expr $(echo "scale=0; $time_fcst/($nowh*3600)" | bc) % 2` = 0 ]; then
        cpfs fort.68.nc ${RUN}.prep.68.nc
     else
        cpfs fort.67.nc ${RUN}.prep.68.nc
     fi
   	      
     if [ $SENDCOM = YES ]; then
        if [ $hotstart -lt 3 ]; then
           ...
           cpfs ${RUN}.prep.68.nc      $COMOUTrerun/${RUN}_prep.68.nc
        else
  ...
  msg="Completing stofs_2d_glo_prep_forecast script"
  echo "$msg"
  postmsg "$jlogfile" "$msg"

# End of stofs_2d_glo_prep_forecast.sh.ecf script --------------------------- #
